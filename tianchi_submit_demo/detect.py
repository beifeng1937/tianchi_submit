...
def detect(save_img=False):
    ...
    model = torch.load(
        weights, map_location=device)['model'].float().eval()  # load FP32 model
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in det:
                    ...
                    if save_json:
                        ...
                        x1, y1, x2, y2 = float(xyxy[0]), float(xyxy[1]), float(
                            xyxy[2]), float(xyxy[3])
                        bbox = [x1, y1, x2, y2]
                        img_name = name
                        conf = float(conf)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--weights', type=str, default='weights/best.pt', help='model.pt path')
    parser.add_argument(
        '--save_dir', type=str, default='./', help='result save dir')
    # parser.add_argument('--source', type=str, default='convertor/fold0/images/val', help='source')  # file/folder, 0 for webcam
    parser.add_argument(
        '--source',
        type=str,
        default=r'tcdata/guangdong1_round2_testB_20191024',
        help='source')  # file/folder, 0 for webcam
    parser.add_argument(
        '--output', type=str, default='inference/output',
    ...
    parser.add_argument(
        '--iou-thres', type=float, default=0.05, help='IOU threshold for NMS')
    parser.add_argument(
